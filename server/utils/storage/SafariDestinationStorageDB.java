...
public class SafariDestinationStorageDB implements SafariDestinationStorage{
		...
		public void put(SafariDestination sd){
			...
			System.out.println(sql);
				...
				String sql = String.format("UPDATE SAFARIDESTINATION SET MAX_PARTICIPANTS=%d, TERRAIN='%s', EQUIPMENT_REQ='%s', LOCATION='%s', ACTIVE=%d WHERE LOCATION=%s)",sd.getMaxParticipants()
																																											,sd.getTerrain()
																																											,sd.getEquipmentReq()
																																											,sd.getGuide()
																																											,sd.getLocation()
